...
<body> 
    <?php
        ...
        echo '<h1 class = "titulo texto"> Editar dados do anúncio </h1>';
        ...
        if($resultadoCodAnuncio->num_rows > 0){
            while($rowCodAnuncio = mysqli_fetch_assoc($resultadoCodAnuncio)){
                ...
                <td>
                    ...
                    <div> <a style = 'margin-left:10px;' href = '../sistemas/sistema_excluir_foto_anuncio.php?id_anuncio=$id_anuncio&id=$id_foto'> Remover imagem </a> </div>             
                </td>";
            }
        }
        ...
        $teste = "select * from tb_anuncio where id_anuncio = '$id_anuncio'";
        $resultado = mysqli_query($conn, $teste);
...
<form class = "enviarArquivo" action="tela_editar_livro.php?id_anuncio=<?php echo $id_anuncio; ?>" method = "POST"  enctype="multipart/form-data"> 
...
<form class = "enviarArquivo" action="tela_cadastro_foto_livro.php" method = "POST"> 
    ...
    <input type = "radio" id = "estrela_1" name = "avaliacao" value = "1" required <?php if($avaliacao==1){echo "checked";} ?>>
    <label for="estrela_1" class = "estrelas" > 1  </label> 

    <input type = "radio"  id = "estrela_2"  name = "avaliacao" value = "2" required <?php if($avaliacao==2){echo "checked";} ?>>
    <label for="estrela_2" class = "estrelas"> 2  </label> 

    <input type = "radio" id = "estrela_3"  name = "avaliacao" value = "3" required <?php if($avaliacao==3){echo "checked";} ?>>
    <label for="estrela_3" class = "estrelas" > 3  </label> 

    <input type = "radio" id = "estrela_4"  name = "avaliacao" value = "4" required <?php if($avaliacao==4){echo "checked";} ?>>
    <label for="estrela_4" class = "estrelas" > 4  </label> 

    <input type = "radio" id = "estrela_5"  name = "avaliacao" value = "5" required <?php if($avaliacao==5){echo "checked";} ?>>
    ...
    <textarea id = "descricao_anuncio" name = "descricao_anuncio" style = "width:300px;height:150px;" required> <?php if(isset($descricao_anuncio)){echo $descricao_anuncio;} ?> </textarea> <br>
